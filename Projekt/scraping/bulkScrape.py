...
def getGeneralInfo(soup):
    ...
    for attribute in attributes:
        ...
        # Switch on the name (does not work with the python version required for Hopsworks... >:()
        if name == 'BRF':
            # If the attribute is a BRF, get the link to the page
            link = attribute.find('a').get('href')
            data['brfLink'] = link
            data[name] = value
        elif name in ['Slutpris', 'Utropspris']:
            data[name] = uf.cleanPrice(value)
        elif name == 'Prisutveckling':
            data[name] = uf.cleanDiff(value)
        elif name in ['Avgift', 'Driftskostnad']:
            data[name] = uf.cleanAvgift(value)
        elif name == 'Sista bud':
            data['Slutpris'] = uf.cleanPrice(value)
        elif name in ['Dagar på Booli', 'Slutpris/m²', 'Storlek', 'Tomtstorlek', 'Upplåtelseform', 'Sidvisningar', 'Dagar som snart till salu', 'Biarea', 'Boendekostnad', 'Kvadratmeterpris', 'Såld eller borttagen']:
            pass
        else:
            data[name] = value
